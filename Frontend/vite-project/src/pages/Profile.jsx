...
import ConnectionButton from '../component/ConnectionButton.jsx';
...
function Profile() {
  ...
  />
          ...
          <div className='mt-[30px] pl-[20px]  font-semibold text-gray-700'>
            ...
            <div className='text-[16px] text-gray-500'>{`${profileData?.connection?.length || 0} connection`}</div>
          </div>
          {profileData?._id === userData?._id &&
            <button className='min-w-[150px] h-[40px] my-[20px] rounded-full border-2 ml-[20px] border-[#2dc0ff] text-[#2dc0ff] flex items-center justify-center gap-[10px]' onClick={() => setEdit(true)}>Edit Profile <HiPencil /></button>}
  ...
  <div className="ml-[20px] mt-[20px]">
    <ConnectionButton userId={profileData._id} />
        ...
        {profilePost.map((post, index) => (
          <Post key={index} id={post._id} description={post.description} author={post.author} image={post.image} like={post.like} comments={post.comments} createdAt={post.createdAt} />
        ))}
        ...
        {profileData?.education?.length > 0 &&
          <div className='w-full min-h-[100px] flex flex-col gap-[10px] justify-center p-[20px] font-semibold bg-white shadow-lg rounded-lg'>
            ...
            <div className='flex flex-col justify-start items-start gap-[20px] text-gray-600 p-[20px]'>
              ...
              {profileData._id === userData._id &&
                <button className='min-w-[150px] h-[40px] rounded-full border-2 border-[#2dc0ff] text-[#2dc0ff] flex items-center justify-center gap-[10px]' onClick={() => setEdit(true)}>Add Education</button>}
            </div>
          </div>
        }
        ...
        {profileData?.experience?.length > 0 &&
          <div className='w-full min-h-[100px] flex flex-col gap-[10px] justify-center p-[20px] font-semibold bg-white shadow-lg rounded-lg'>
            ...
            <div className='flex flex-col justify-start items-start gap-[20px] text-gray-600 p-[20px]'>
              ...
              {profileData._id === userData._id &&
                <button className='min-w-[150px] h-[40px] rounded-full border-2 border-[#2dc0ff] text-[#2dc0ff] flex items-center justify-center gap-[10px]' onClick={() => setEdit(true)}>Add Experience</button>}
            </div>
          </div>
        }
